...
const Hero = () => {
    ...
    useEffect(() => {
        // Set isAnimating to false to start the exit animation
        setIsAnimating(false); 

        // After a brief delay (shorter than the image fade-out)
        const contentExitTimer = setTimeout(() => {
            // Then, transition to the next slide
            setCurrentSlide((prev) => (prev + 1) % slides.length); 
        }, 800); 

        // After the image has transitioned, start the new content's entry animation
        const contentEntryTimer = setTimeout(() => {
            setIsAnimating(true);
        }, 1200); 

        // Set up the interval for the next cycle
        ...
        }, 10000); // 4 seconds total cycle time
